...
if __name__ == '__main__':
    ...
    print("\nTRSensor LCD Test Program ...\r\n")
    TRS = TRSensor()
    lcd = ST7789()
    
    # Initialize LCD
    lcd.fill(lcd.BLACK)
    lcd.text("TR Sensor Test", 60, 5, lcd.WHITE)
    lcd.text("Raw Values:", 10, 25, lcd.YELLOW)
    lcd.show()
    
    # Create visual bars for each sensor
    bar_x = [20, 60, 100, 140, 180]  # X positions for 5 sensors
    bar_width = 30
    bar_max_height = 80
    
    while True:
        values = TRS.AnalogRead()
        
        # Clear the sensor area
        lcd.fill_rect(0, 45, 240, 90, lcd.BLACK)
        
        # Display raw values as text
        for i in range(5):
            lcd.text(str(values[i]), bar_x[i], 45, lcd.WHITE)
        
        # Display visual bars
        for i in range(5):
            # Calculate bar height (invert since high values = dark)
            bar_height = int((values[i] / 1023) * bar_max_height)
            
            # Draw bar
            if bar_height > 0:
                color = lcd.GREEN if values[i] < 500 else lcd.RED if values[i] > 900 else lcd.YELLOW
                lcd.fill_rect(bar_x[i], 130 - bar_height, bar_width, bar_height, color)
            
            # Draw bar outline
            lcd.rect(bar_x[i], 50, bar_width, bar_max_height, lcd.WHITE)
        
        # Show sensor labels
        for i in range(5):
            lcd.text(f"S{i}", bar_x[i] + 10, 133, lcd.WHITE)
